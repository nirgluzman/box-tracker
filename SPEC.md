...
- Edit opens a **modal overlay** (centered card on desktop, bottom sheet on mobile), prefilled with current values. It is rendered once at the screen level rather than inline in the list, so it always appears in view regardless of scroll position in a long list (no scrolling to the row). Closes via the × (top-right), the Cancel button, Escape, or the Android back button; the backdrop is deliberately **not** click-to-dismiss, since a stray tap outside a form must not discard typed edits. The form can also add photos (rear camera or gallery) and remove existing photos on the box, which persist immediately (section 13 — the "type now, add photos later" flow).
