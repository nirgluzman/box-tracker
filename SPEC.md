...
- Photo button opens camera or gallery, uploads to Storage under `boxPhotos/{docId}/...`, appends the download URL to photoUrls. The rear-camera "Take photo" button (`capture='environment'`) is shown on touch devices only (detected via `pointer: coarse`); on laptops/desktops it is hidden, since there is no useful rear camera and `capture` is ignored - "Gallery" upload and voice input remain available there. Same rule applies to the Edit form on Browse.
