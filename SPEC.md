...
- The goal is a tight box-contents label, not a prose summary: extract only the physical items packed, as a concise comma-separated list, dropping filler, repetitions, hesitations, side comments, and background talk. Output language is **Hebrew**: if the transcript is in English or German, translate the items to Hebrew. Exception — keep individual English/German words that are normally used as-is in Hebrew (brand names, proper nouns, model names, common loanwords) in their original form; don't force-translate those. Output only the list — no preamble or quotes. If no items are found, return the transcript with filler removed. Implemented as a system prompt:
...
form; do not force-translate those. Remove filler words, repetitions,
