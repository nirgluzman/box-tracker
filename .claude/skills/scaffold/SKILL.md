---
name: scaffold
description: Scaffold the BoxIndex (box-tracker) Vite + React + TypeScript + Firebase project from SPEC.md. Use only on the initial empty repo before any code exists.
...
8. **vite-plugin-pwa:** configure manifest (name/short_name "BoxIndex", icons from `public/icons/`, theme color) and a service worker with cache-first runtime caching for Firebase Storage photo URLs (SPEC 13).
